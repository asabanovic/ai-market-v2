...
def api_admin_stats():
    ...
    try:
        ...
        # Get pending/processing receipts count (unprocessed)
        from models import Receipt, SupportMessage
        pending_receipts = db.session.query(Receipt).filter(
...
def api_admin_user_profile(user_id):
    ...
    try:
        ...
        user_data = {
            ...
            'is_pwa_user': target_user.is_pwa_user,
            'last_pwa_access': target_user.last_pwa_access.isoformat() if target_user.last_pwa_access else None,
            'pwa_access_count': target_user.pwa_access_count,
