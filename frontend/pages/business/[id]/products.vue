        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <div v-if="products.length > 0" class="flex gap-2">
            <button
              ...
              class="bg-yellow-500 text-white px-4 py-2 rounded-md font-medium hover:bg-yellow-600 transition duration-200 flex items-center"
            >
              ...
              <tr v-for="product in filteredProducts" :key="product.id" class="product-row">
                ...
                <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                  <button
                    ...
                    class="mr-2"
                    :title="featuredProducts.includes(product.id) ? 'Ukloni iz Featured' : 'Dodaj u Featured'"
                  >
                    <svg
                      class="w-5 h-5 transition-colors"
                      :class="featuredProducts.includes(product.id) ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300 hover:text-yellow-400'"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-gray-500">
                  <span v-if="product.discount_price" class="text-green-600 font-semibold">{{ formatPrice(product.discount_price) }} KM</span>
                  <span v-else class="font-semibold">{{ formatPrice(product.base_price) }} KM</span>
                </p>
              </div>
              <div class="flex-shrink-0">
                <svg
...
async function loadFeaturedProducts() {
  try {
    const data = await get(`/api/admin/businesses/${businessId.value}/featured`)
...
async function loadAllProductsForFeatured() {
  ...
  try {
    // Load all products (no pagination) for the featured modal
    const data = await get(`/api/businesses/${businessId.value}/products?per_page=1000`)
...
function toggleFeatured(productId: number) {
  if (featuredProducts.value.includes(productId)) {
    ...
    showNotification('Maksimalno 6 featured proizvoda', 'error')
    return
  }
  // Auto-save when toggling from the product row
...
async function saveFeaturedProducts() {
  ...
  try {
    await post(`/api/admin/businesses/${businessId.value}/featured`, {
      product_ids: featuredProducts.value
    })
    showNotification('Featured proizvodi spremljeni', 'success')
  } catch (error) {
    showNotification('Greska pri spremanju featured proizvoda', 'error')
  } finally {
