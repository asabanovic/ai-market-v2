  ...
  <div class="rounded-lg shadow-md overflow-hidden relative hover:shadow-xl transition-shadow duration-300" :class="[product.is_teaser ? 'opacity-90' : '', hasActiveDiscount ? 'bg-green-200 ring-2 ring-green-500' : hasUpcomingDiscount ? 'bg-yellow-100 ring-2 ring-yellow-400' : product.contributor_name ? 'bg-white ring-2 ring-purple-400' : 'bg-white']">
    ...
    <div class="h-48 bg-white flex items-center justify-center cursor-pointer pt-6 relative" @click="showDetails">
      ...
      <div
        ...
        class="absolute bottom-0 left-0 right-0 z-20 bg-gradient-to-t from-black/95 via-black/70 to-transparent px-3 py-3 pt-6"
      >
        <div class="flex items-center gap-1.5 text-amber-400 text-xs font-light">
          <div class="w-5 h-5 rounded-full bg-amber-500/30 flex items-center justify-center flex-shrink-0 ring-1 ring-amber-400/50">
            <Icon name="mdi:account" class="w-3 h-3 text-amber-400" />
          </div>
          <span>
            Dodao/la <span class="font-medium text-amber-300">{{ product.contributor_name }}</span>
          </span>
        </div>
      </div>
    </div>
